...
function setup(){
 ...
 house2=createSprite(windowWidth+6500,windowHeight-300,20,20)
  ...
  weapon2 = createSprite(windowWidth+6500,windowHeight-150,20,20);
    ...
    weapon2.scale=0.6
    crown_spr = createSprite(windowWidth+10000,windowHeight-150,20,20);
    ...
    instruction = createSprite(windowWidth-1250,windowHeight-400,50,50)
...
function draw(){
 ...
 if(game === 1){
    ...
    fill("green")
    stroke("orange")
    text("Use Arrow Keys 'Right' to move right And 'Left' to move left",camera.position.x,windowHeight-700)
    text("Collect the Weapon from the house to shoot at enemies",camera.position.x,windowHeight-650)
    text("Collect The 'CROWN' from the last house to win the game",camera.position.x-500,windowHeight-750)
    createPlayer2(); 
}
    ...
    if(keyIsDown(RIGHT_ARROW)&& shootWeapon===1 ||touches.length>0 && shootWeapon === 0){
        heart1.x+=10
        heart2.x+=10
        heart3.x+=10
        soldier.x+=10
...
else  if(keyIsDown(LEFT_ARROW)&& shootWeapon===1 ||touches.length>0 && shootWeapon === 0){
    heart1.x-=10
        heart2.x-=10
        heart3.x-=10 
        soldier.x-=10
    ...
    if(mousePressedOver(door1)){
            ...
            if(flag1===1){
                image(inside1_img,windowWidth+1000,windowHeight-windowHeight,width,height);
                house1.destroy();
                door1.destroy();
    ...
    if(shootWeapon===1 && keyDown("Space")){
...
fill("green")
stroke("orange")
text("Game will start in 20 seconds",camera.position.x-750,windowHeight-400)
...
function myFunc(){
    ...
    instruction.destroy();
    game = 1
    if(keyIsDown(RIGHT_ARROW) && shootWeapon === 0){
        heart1.x+=10
        heart2.x+=10
        heart3.x+=10
        soldier.x+=10
...
else  if(keyIsDown(LEFT_ARROW)  && shootWeapon === 0){
    heart1.x-=10
    heart2.x-=10
    heart3.x-=10    
    soldier.x-=10
